    ...
    noteList.forEach(async function(xnote) {
      swap.forEach(async function(key, value) {
        if (xnote._id == key.new_id) {
          ...
          var image_url = xnote.image;
          console.log('\n1 image_url ' + image_url);
          const ele = image_url.split('/');
          image_url = image_url.replace(ele[10], key.new_id);
          console.log('2 image url ' + image_url);
          var new_note = await new Note({
            title: xnote.title,
            lectureNote: xnote.lectureNote,
            creationDate: xnote.creationDate,
            updateDate: xnote.updateDate,
            keywords: xnote.keywords,
            questions: xnote.questions,
            comments: xnote.comments,
            summary: xnote.summary,
            subject: xnote.subject,
            topic: xnote.topic,
            subtopic: xnote.subtopic,
            image: image_url,
            _id:xnote._id // This is required, or a new ID will be assigned!
          });
          Note.findByIdAndUpdate(xnote._id, new_note, {}, function (err, thenote) {
              if (err) {
                console.log(err);
                return 1;
              }
